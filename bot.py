...
async def date_dialog(update, context):
    text = update.message.text
    message = await send_text(update, context, "Девушка набирает текст...")
...
async def date_button(update, context):
    ...
    await send_text(update, context, "Отличный выбор! Пригласите этого человека на свидание за 5 сообщений")
...
async def message_button(update, context):
    ...
    prompt = load_prompt(query)
    user_chat_history = "\n\n".join(dialog.list)
    message = await send_text(update, context, "Чат-GPT думает над вариантом ответа...")
...
async def profile_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        message = await send_text(update, context, "ChatGPT занимается генерацией вашего профиля. Подождите...")
